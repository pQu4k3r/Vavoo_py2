...
class ProxyHealthMonitor:
    ...
    def _restart_proxy(self):
        ...
        try:
            ...
            try:
                requests.get(
                    "http://127.0.0.1:" + str(PORT) + "/shutdown",
                    timeout=2
                )
                time.sleep(2)
            except Exception:
            ...
            print(
                "[Health Monitor] Failed to restart proxy: " +
                str(e)
            )
...
def start_proxy():
    ...
    while restart_count < max_restarts:
        try:
            ...
            server = ThreadingHTTPServer(('0.0.0.0', PORT), VavooHTTPHandler)
            server.timeout = 30
    ...
    for i in range(15):  # 15 attempts
        if is_proxy_running():
            try:
                # Health check
                response = requests.get(
                    "http://127.0.0.1:" + str(PORT) + "/status", timeout=3)
                if response.status_code == 200:
                    data = response.json()
